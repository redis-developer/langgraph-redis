...
def test_standard_key_parsing():
    ...
    assert len(result) == 5
    assert set(result.keys()) == {"thread_id", "checkpoint_ns", "checkpoint_id", "task_id", "idx"}
...
def test_incorrect_prefix():
    ...
    # Attempt to parse the key - should raise ValueError
    with pytest.raises(ValueError, match="Expected checkpoint key to start with 'checkpoint'"):
        BaseRedisSaver._parse_redis_checkpoint_writes_key(key)