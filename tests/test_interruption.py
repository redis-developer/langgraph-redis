    ...
    def __getattr__(self, name):
        ...
        if callable(attr) and name == self.interrupt_on:
            ...
            async def wrapper(*args, **kwargs):
                ...
                if name in self.interrupt_after_count and self.operations_count[name] >= self.interrupt_after_count[name]:
                    raise InterruptionError(f"Simulated interruption during {name} operation")
    ...
    def __getattr__(self, name):
        ...
        if callable(attr) and operation_name == self.parent_mock.interrupt_on:
            ...
            async def wrapper(*args, **kwargs):
                ...
                # Check if we should interrupt
                if (operation_name in self.parent_mock.interrupt_after_count and 
                    self.parent_mock.operations_count[operation_name] >= self.parent_mock.interrupt_after_count[operation_name]):
                    raise InterruptionError(f"Simulated interruption during {operation_name} operation")
        ...
        # Special handling for pipeline method to track operations within the pipeline
            ...
            async def execute_wrapper(*args, **kwargs):
                ...
                if self.parent_mock.interrupt_on == "Pipeline.execute":
                    ...
                    if ("Pipeline.execute" in self.parent_mock.interrupt_after_count and 
                        self.parent_mock.operations_count["Pipeline.execute"] >= self.parent_mock.interrupt_after_count["Pipeline.execute"]):
                        raise InterruptionError(f"Simulated interruption during Pipeline.execute operation")
...
def create_test_checkpoint() -> tuple[RunnableConfig, Checkpoint, CheckpointMetadata, Dict[str, str]]:
...
def verify_checkpoint_state(redis_client: Redis, thread_id: str, checkpoint_id: str, expected_present: bool = True) -> None:
    ...
    assert (len(keys) > 0) == expected_present, f"Expected checkpoint data {'to exist' if expected_present else 'to not exist'}"
    
    if expected_present:
        # Check if specific checkpoint ID exists
        assert any(checkpoint_id.encode() in key or checkpoint_id in key.decode() for key in keys), f"Checkpoint ID {checkpoint_id} not found in Redis"
    ...
    async with create_interruptible_saver(
        ...
        try:
            ...
            if result is not None:
                assert result != checkpoint, "Checkpoint should not be completely saved after interruption"
        finally:
        ...
        try:
            ...
            if checkpoint_tuple and checkpoint_tuple.pending_writes:
                for write in checkpoint_tuple.pending_writes:
                    assert write.channel not in ["channel1", "channel2"], "Transaction should have been rolled back"
        finally:
            # Cleanup
            saver._redis = original_redis
...
async def test_graph_simulation_with_interruption(redis_url: str) -> None:
    ...
    async with AsyncRedisSaver.from_conn_string(redis_url) as saver:
        ...
        assert current and current["id"] == initial_checkpoint["id"], "Should still have initial checkpoint after transaction abort"
